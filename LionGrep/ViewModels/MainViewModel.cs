...
public partial class MainViewModel : ObservableObject
{
    ...
    [ObservableProperty] private string _windowTitle = "LionGrep";
    ...
    private async Task RunSearchAsync(bool invert, IReadOnlyList<string>? restrictToPaths)
    {
        ...
        if (!isFileList)
        {
            ...
            roots = LionGrep.Core.Logic.PathPrefixDedup.Remove(rawRoots);
            if (roots.Count == 0)
            {
    ...
    private void UpdateWindowTitle()
    {
        var firstRoot = SearchIn?
            .Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .FirstOrDefault();
        WindowTitle = string.IsNullOrWhiteSpace(firstRoot) ? "LionGrep" : $"LionGrep — {firstRoot}";
    }
