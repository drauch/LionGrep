...
Engine-level micro-benchmarks for `LionGrep.Core`, plus a deterministic synthetic-corpus
...
```pwsh
dotnet run --project LionGrep.Bench -c Release -- --filter '*'
```
...
```pwsh
# A single benchmark
dotnet run --project LionGrep.Bench -c Release -- --filter '*RegexWithPrefilter*'

# Just the literal-search ones
dotnet run --project LionGrep.Bench -c Release -- --filter '*Literal*'

# List without running
dotnet run --project LionGrep.Bench -c Release -- --list flat
```
...
# 3) Time LionGrep. The corresponding micro-benchmark is `LiteralCaseSensitive`. Note that BDN
...
```pwsh
Remove-Item -Recurse "$env:TEMP\liongrep-bench-*"
```
